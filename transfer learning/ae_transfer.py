...
if args.compress != None:
    ...
    initialize_bitmap_start = time.time()
    bitmap = Bitmap(data_num_size)
    initialize_bitmap_end = time.time()

    setup_tensorflow_start = time.time()
    ...
    setup_tensorflow_end = time.time()

    sess = tf.Session(config=tf.ConfigProto(allow_soft_placement=True, log_device_placement=True))

    restore_saved_checkpoint_start = time.time()
    saver.restore(sess, "./wb.ckpt")
    restore_saved_checkpoint_end = time.time()

    write_preliminary_info_start = time.time()
    ...
    write_preliminary_info_end = time.time()
    ...
    # Open a file to write the error values to
    # This file has the same name as the training file with ".error" at the end
    arrays_and_files_start = time.time()
    error_log = open(file_name + ".error", 'w')
    ...
    arrays_and_files_end = time.time()
    ...
    bo_init_start = time.time()
    batch_xs = [[0 for x in range(n_input)] for y in range(batch_size)]  # Initialize batch_xs to be filled with zeros
    original_values = [[0 for x in range(n_input)] for y in range(batch_size)]  # Initialize original_values to be filled with zeros
    bo_init_end = time.time()
    ...
    write_final_info_start = time.time()
    ...
    zfile.close()
    write_final_info_end = time.time()

    print("For the testing data set, Error_0: %.8f\tError_A: %.8f\tError_B: %.8f\tError_C: %.8f\tError_D: %.8f\tError_E: %.8f\tError_F: %.8f\tError_mean: %.8f\t" % ((error_0 / data_num.size), (error_A / data_num.size), (error_B / data_num.size), (error_C / data_num.size), (error_D / data_num.size), (error_E / data_num.size), (error_F / data_num.size), (error_sum / data_num.size)), file=error_log)

    get_sizes_start = time.time()
    ...
    print()
    print("Total compression time: %f seconds" % (compression_end - compression_start))
    print("Checking whether the file exists: %f seconds" % (file_exists_end - file_exists_start))
    print("Get Data Method: %f seconds" % (get_data_end - get_data_start))
    print("Normalize Data Method: %f seconds" % (normalize_data_end - normalize_data_start))
    print("Initialize Bitmap: %f seconds" % (initialize_bitmap_end - initialize_bitmap_start))
    print("Setup TensorFlow: %f seconds" % (setup_tensorflow_end - setup_tensorflow_start))
    print("Restore Saved Checkpoint: %f seconds" % (restore_saved_checkpoint_end - restore_saved_checkpoint_start))
    print("Write Preliminary Information: %f seconds" % (write_preliminary_info_end - write_preliminary_info_start))
    print("Setup Modifications: %f seconds" % (setup_modifications_end - setup_modifications_start))
    print("Setup Arrays and Open Files: %f seconds" % (arrays_and_files_end - arrays_and_files_start))
    print("Initializing batch_xs and original_values: %f seconds" % (bo_init_end - bo_init_start))
    print("Checking whether it's the last total_batch: %f seconds" % last_check_sum)
    print("Filling batch_xs and original_values: %f seconds" % fill_bo_sum)
    print("Getting the z-layer: %f seconds" % get_z_layer_sum)
    print("Setup z: %f seconds" % setup_z_sum)
    print("p_time: %f seconds" % p_sum)
    print("p_proper_time: %f seconds" % p_proper_sum)
    print("cdt_time: %f seconds" % cdt_sum)
    print("Bitmap, Delta, and Error Processing: %f seconds" % bitmap_delta_error_processing_sum)
    print("Error Organizing Time: %f seconds" % error_organizing_sum)
    print("Write Final Information: %f seconds" % (write_final_info_end - write_final_info_start))
    print("Getting File Sizes: %f seconds" % (get_sizes_end - get_sizes_start))
