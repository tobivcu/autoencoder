...
def normalize_data(data_num):
    ...
    modifications_all = [0] * data_num_size    # This list should store the number of times that each number in the dataset was multiplied or divided by 10
...
if args.compress != None:
    ...
    bitmap = Bitmap(data_num_size)    # Indicates which indices have out-of-error-bound predictions
    num_ooe_pos = 0
    num_ooe_neg = 0
    ...
    ppoints_before_undoing_normalization = array.array('d', [0.0] * data_num_size)
    ...
    # for new stuff
    new_dipoints = array.array('I')
    new_diname = file_name + ".new_dindex"
    new_difile = open(new_diname, 'wb')

    # for newer stuff
    dipoints3 = array.array('I')
    diname3 = file_name + ".dindex3"
    difile3 = open(diname3, 'wb')
    ...
    sum_ooe_original_pos = 0.0      # Sum of the original values after normalization whose predicted values have out-of-bound errors
    sum_ooe_output_pos = 0.0        # Sum of the predicted values before undoing the normalization for values that have out-of-bound errors
    ...
    for i in range(total_batch):
        ...
        for r in range(np.size(p, 0)):
            for s in range(np.size(p, 1)):
                try: # Ensure that the index is not out-of-range
                    ppoints_before_undoing_normalization[current_index] = p[r][s]
    ...
    # If there is an error-bound 12-2-2019
    if args.error != None:
        ...
        new_prediction = [0 for x in range(data_num_size)]
        ...
        for a in range(data_num_size):    # Go through every element in the dataset
            if bitmap.test(a):    # If this index has an out-of-error-bound prediction
                ...
                if abs((new_prediction[a] - original_data_num[a]) / original_data_num[a]) > error_bound:    # If still out-of-error-bound after adding mean_diff
                    ...
                    dvpoints.append(data_num[a] - new_prediction[a])
                index_of_bad_predictions += 1
            else:
                new_prediction[a] = ppoints[a]
    ...
    for new_item in new_bitmap.array:
        new_dipoints.append(new_item)

    new_dipoints.tofile(new_difile)
    new_difile.close()

    for item3 in bitmap3.array:
        dipoints3.append(item3)

    dipoints3.tofile(difile3)
    difile3.close()
    ...
    if args.output_error:
        for i in range(data_num_size):
                try:    # Ensure that the index is not out-of-range
                    # If the true value is not 0
                    if original_data_num[i] != 0:
                        error = (abs(new_prediction[i] - original_data_num[i]) / original_data_num[i])
    ...
    if args.output_error:
        # Print error information to the error log file
